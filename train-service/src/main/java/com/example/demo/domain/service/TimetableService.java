...
import com.example.demo.domain.shared.dto.SettingQueriedView;
import com.example.demo.domain.shared.dto.TemplateQueriedView;
import com.example.demo.domain.shared.dto.TimetableGeneratedView;
import com.example.demo.domain.shared.dto.TimetableGeneratedView.TimetableDetailGeneratedView;
...
public class TimetableService extends BaseDomainService {
	 ...
	 */
	public TimetableGeneratedView generateTimetableByFromStopAndToStop(List<GenerateTimetableCommand> commands) {
		// 取得火車時刻表範本資訊
		...
		TemplateQueriedView templateData = this.transformAggregate(template, TemplateQueriedView.class);

		List<TimetableDetailGeneratedView> details = BaseDataTransformer.transformData(commands,
				TimetableDetailGeneratedView.class);
		...
		List<SettingQueriedView> parameterList = this.transformAggregate(settings, SettingQueriedView.class);
		Map<String, Object> parameters = parameterList.stream()
				.collect(Collectors.toMap(SettingQueriedView::getName, SettingQueriedView::getValue));
		// 設置 Detail 大小
		parameters.put("detailSize", details.size());

		TimetableGeneratedView timetableGeneratedData = new TimetableGeneratedView();
