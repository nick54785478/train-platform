CREATE TABLE TRAIN (
    UUID CHAR(36) NOT NULL PRIMARY KEY,
    TRAIN_NO INT,
    TRAIN_KIND VARCHAR(255),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE TRAIN_STOP (
    UUID CHAR(36) NOT NULL PRIMARY KEY,
    TRAIN_UUID CHAR(36),
    SEQ INT,
    NAME VARCHAR(255),
    TIME TIME,
    DELETE_FLAG VARCHAR(255),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255),
    CONSTRAINT FK_TRAIN_STOP_TRAIN FOREIGN KEY (TRAIN_UUID) REFERENCES TRAIN(UUID)
);

CREATE TABLE TRAIN_TICKET (
    ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    TICKET_NO VARCHAR(255) NOT NULL,
    TYPE VARCHAR(40) NOT NULL,
    TRAIN_UUID VARCHAR(255) NOT NULL,
    FROM_STOP VARCHAR(255) NOT NULL,
    TO_STOP VARCHAR(255) NOT NULL,
    PRICE DECIMAL(19, 2) NOT NULL,
    CREATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE TRAIN_SEAT (
    ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    TICKET_UUID CHAR(36) NOT NULL,
    TRAIN_UUID CHAR(36) NOT NULL,
    BOOK_UUID CHAR(36) NOT NULL,
    CAR_NO BIGINT NOT NULL,
    TAKE_DATE DATE,
    SEAT_NO VARCHAR(255),
    BOOKED CHAR(2),
    ACTIVE_FLAG CHAR(2),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE TICKET_BOOKING (
    UUID CHAR(36) NOT NULL PRIMARY KEY,
    USERNAME VARCHAR(255) NOT NULL,
    EMAIL VARCHAR(255) NOT NULL,
    TRAIN_UUID CHAR(36) NOT NULL,
    TICKET_UUID CHAR(36) NOT NULL,
    ACCOUNT_UUID CHAR(36) NOT NULL,
    TAKE_STATUS VARCHAR(100) NOT NULL,
    STATUS VARCHAR(100) NOT NULL,
    ACTIVE_FLAG CHAR(2),
    CREATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE IF NOT EXISTS `EVENT_LOG` (
  ID BIGINT(20) NOT NULL AUTO_INCREMENT,
  UUID CHAR(36) NOT NULL,
  TX_ID CHAR(36) NOT NULL,
  USER_ID VARCHAR(32) NOT NULL,
  EVENT_CLASS_NAME VARCHAR(255) NOT NULL,
  OCCURED_AT DATETIME NOT NULL,
  TOPIC VARCHAR(100),
  TARGET_ID CHAR(36),
  BODY LONGTEXT NOT NULL,
  SEND_QUEUE_STATUS VARCHAR(30) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `EVENT_IDEMPOTENT_LOG` (
  EVENT_TYPE VARCHAR(190) NOT NULL,
  UNIQUE_KEY VARCHAR(64) NOT NULL,
  TARGET_ID CHAR(36),
  CREATED_DATE TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (EVENT_TYPE, UNIQUE_KEY),
  KEY `idx_type_key` (`EVENT_TYPE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;


CREATE TABLE TRAIN (
    UUID CHAR(36) NOT NULL PRIMARY KEY,
    TRAIN_NO INT,
    TRAIN_KIND VARCHAR(255),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE MONEY_ACCOUNT (
    UUID CHAR(36) NOT NULL PRIMARY KEY,
    USERNAME VARCHAR(255) NOT NULL,
    NAME VARCHAR(255) NOT NULL,
    EMAIL VARCHAR(255)  NOT NULL,
	BALANCE DECIMAL(19, 2),
	ACCOUNT_STATUS VARCHAR(100) NOT NULL,
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
);


CREATE TABLE IF NOT EXISTS `EVENT_SOURCE` (
  ID BIGINT AUTO_INCREMENT PRIMARY KEY,
  UUID CHAR(100) NOT NULL,
  AGGREGATE_ID CHAR(36) NOT NULL,
  USER_ID VARCHAR(32) NOT NULL,
  EVENT_CLASS_NAME VARCHAR(255) NOT NULL,
  OCCURED_AT DATETIME NOT NULL,
  BODY LONGTEXT NOT NULL,
  STATUS CHAR(20) NOT NULL,
  VERSION BIGINT NOT NULL,
  CONSTRAINT unique_target_id_version UNIQUE (AGGREGATE_ID, VERSION)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS SETTING (
    ID BIGINT(20) AUTO_INCREMENT  PRIMARY KEY,
    DATA_TYPE VARCHAR(20),
    TYPE VARCHAR(100),
    NAME VARCHAR(100),
    VALUE TEXT,
    DESCRIPTION TEXT,
    PRIORITY_NO BIGINT,
    ACTIVE_FLAG CHAR(1)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE TEMPLATE (
	ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    NAME VARCHAR(255) NOT NULL,
    TEMPLATE_TYPE VARCHAR(255) NOT NULL,
    FILE_TYPE VARCHAR(30) NOT NULL,
    FILE_PATH TEXT NOT NULL,
    `FILE_NAME`  VARCHAR(255) NOT NULL,
    DELETE_FLAG CHAR(1),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
)

CREATE TABLE CUSTOMISATION (
    ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    USERNAME VARCHAR(100),
    `TYPE` VARCHAR(100),
    NAME VARCHAR(100),
    `VALUE` VARCHAR(150),
    DESCRIPTION TEXT,
    ACTIVE_FLAG CHAR(2),
    CREATED_DATE DATETIME,
    CREATED_BY VARCHAR(255),
    LAST_UPDATED_DATE DATETIME,
    LAST_UPDATED_BY VARCHAR(255)
);

CREATE TABLE `BUSINESS_IDEMPOTENCY_LOG` (
    `ID` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `BUSINESS_KEY` VARCHAR(255) NOT NULL COMMENT '業務唯一 Key (例如 SEAT_CLAIM:...)',
    `TARGET_ID` VARCHAR(100) DEFAULT NULL COMMENT '目標識別碼 (例如 座位號)',
    `CREATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY `uk_business_key` (`BUSINESS_KEY`) -- 這是冪等性的核心守門員
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE `EMAIL_TEMPLATE` (
    `ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主鍵 ID',
    `TEMPLATE_KEY` VARCHAR(100) NOT NULL COMMENT '範本唯一識別碼',
    `SUBJECT` VARCHAR(255) NOT NULL COMMENT '郵件主旨',
    `CONTENT` TEXT NOT NULL COMMENT '郵件 HTML 內容',
    `ACTIVE_FLAG` VARCHAR(1) NOT NULL DEFAULT 'Y' COMMENT '啟用狀態 (Y/N)',
    
    -- 繼承自 BaseAggreagteRoot 的審計欄位
    `CREATED_DATE` DATETIME NOT NULL COMMENT '創建時間',
    `CREATED_BY` VARCHAR(50) DEFAULT NULL COMMENT '創建者',
    `LAST_UPDATED_DATE` DATETIME NOT NULL COMMENT '最後異動時間',
    `LAST_UPDATED_BY` VARCHAR(50) DEFAULT NULL COMMENT '最後異動者',
    
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK_TEMPLATE_KEY` (`TEMPLATE_KEY`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='郵件範本管理表';